...
#define QUANTIDADE_DE_COMPRAS 1
...
int main(void)
{
    ...
    while (compras < QUANTIDADE_DE_COMPRAS)
    {
        ...
        /* Input check */
        if (!scanf_success)
        { // ERRO scanf nao obtem 2 valores
            clear_screen();
        ...
        { // ERRO opcao selecionada invalida
            clear_screen();
        ...
        { // ERRO quantidade de acoes nao disponivel
            clear_screen();
    ...
    print_horizontal_line(border_char, table_width, 1);
    print_horizontal_line(border_char, table_width, 0);
    ...
    print_padded_border(border_char, table_padding, 1);
    for (int i = 0; i < column_amount; i++) {
        print_cell(header[i], column_width, 'l');
    }
    print_padded_border(border_char, table_padding, 0);
    print_horizontal_line(border_char, table_width, 0);
