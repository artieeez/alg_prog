...
void desenha_lista_veiculos(VEICULO lista_veiculos[], bool apagar) {
    for (int i = 0; i < NUM_VEICULOS; i++) {
        ...
        COORDENADA *tmp_envelope[0] = &lista_veiculos[i].envelope;
        COORDENADA *tmp_envelope[1] = &lista_veiculos[i].envelope[1];
        ...
        if (!apagar) {
            if (lista_veiculos[i].dir == DIR) {
                tmp_envelope[0]->x += VEIC_SPEED;
                tmp_envelope[1]->x += VEIC_SPEED;
            ...
            if (tmp_veiculo->valido) {
                if (tmp_envelope[0]->x >= X_MAX && tmp_veiculo->dir == DIR) {
                    tmp_envelope[1]->x -= tmp_envelope[0]->x - 1;
                    tmp_envelope[0]->x = 1;
                } else if (tmp_envelope[1]->x <= X_MIN && tmp_veiculo->dir == ESQ) {
                    tmp_envelope[0]->x += X_MAX;
                    tmp_envelope[1]->x += X_MAX;
                }
            }
        }
    }  // Fim for loop
...
void move_sapo(SAPO *s, DIRECAO_MOVIMENTO dir) {
    ...
    COORDENADA tmp_pos1 = s->envelope;
...
void inicializa_veiculos(VEICULO lista_veiculos[], DIRECAO_MOVIMENTO dir) {
    ...
    for (int i = 0; i < NUM_VEICULOS; i++) {
        VEICULO tmp;
        get_coord_veic(&tmp.envelope[0], tipo, posicao[i]);
        tmp.tipo = ESPORTE;
        tmp.dir = DIR;
        tmp.tamanho = 4;
